...
class TestEndToEndUpload:
    ...
    @pytest.mark.asyncio
    async def test_upload_creates_video_and_transcoding_job(
    ...
    @pytest.mark.asyncio
    async def test_upload_with_category(
        self, admin_client, test_database, test_storage, sample_category
    ):
    ...
    async def test_upload_cleanup_on_failure_preserves_atomicity(
        ...
        """
        initial_video_count = await test_database.fetch_val(
            "SELECT COUNT(*) FROM videos"
        )
        initial_job_count = await test_database.fetch_val(
        ...
        final_video_count = await test_database.fetch_val(
            "SELECT COUNT(*) FROM videos"
        )
        final_job_count = await test_database.fetch_val(
            "SELECT COUNT(*) FROM transcoding_jobs"
        )
...
class TestUploadReUpload:
    ...
    @pytest.mark.asyncio
    async def test_reupload_resets_video_status(
        self, admin_client, test_database, test_storage, sample_video
    ):
...
class TestRetryTranscoding:
    ...
    @pytest.mark.asyncio
    async def test_retry_failed_video_resets_status(
        self, admin_client, test_database, test_storage, sample_category
    ):
    ...
    @pytest.mark.asyncio
    async def test_retry_requires_source_file(
        self, admin_client, test_database, sample_category
    ):
...
class TestBulkRetranscode:
    """Test bulk retranscode operations."""

    @pytest.mark.asyncio
    async def test_bulk_retranscode_queues_videos(
        self, admin_client, test_database, test_storage, sample_category
    ):
        """Test that bulk retranscode queues videos for re-processing."""
        now = datetime.now(timezone.utc)

        # Create multiple ready videos with source files
        video_ids = []
        for i in range(3):
            video_id = await test_database.execute(
                videos.insert().values(
                    title=f"Ready Video {i}",
                    slug=f"ready-video-bulk-{i}",
                    category_id=sample_category["id"],
        ...
        response = admin_client.post(
            ...
            json={"video_ids": video_ids, "qualities": ["all"]},
        )
...
class TestConcurrentUploadRaceCondition:
    ...
    @pytest.mark.asyncio
    async def test_video_and_job_created_before_file_accessible(
        self, admin_client, test_database, test_storage
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_multiple_rapid_uploads_all_get_jobs(
        self, admin_client, test_database, test_storage
    ):
        """
        ...
        for i in range(5):
            ...
            response = admin_client.post(
                "/api/videos",
                files={
                    "file": (f"rapid_{i}.mp4", io.BytesIO(file_content), "video/mp4")
                },
...
class TestProgressTracking:
    ...
    @pytest.mark.asyncio
    async def test_new_upload_has_zero_progress(
        self, admin_client, test_database, test_storage
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_progress_endpoint_returns_job_info(
        self, admin_client, test_database, sample_pending_video
    ):
