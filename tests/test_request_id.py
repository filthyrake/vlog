...
class TestRequestIDMiddleware:
    ...
    def test_request_id_rejects_invalid_characters(self, public_client):
        """Test that request IDs with invalid characters are rejected and regenerated."""
        # Invalid characters like newlines, quotes, etc. should be rejected
        invalid_ids = [
            "id-with\nnewline",
            "id-with\ttab",
            'id-with"quote',
        ...
        for invalid_id in invalid_ids:
            response = public_client.get(
                "/health", headers={"X-Request-ID": invalid_id}
            )
            assert response.status_code in [200, 503]

            # Should have generated a new UUID instead of using the invalid one
            returned_id = response.headers.get("X-Request-ID")
            assert returned_id != invalid_id
            # Verify it's a valid UUID (generated by middleware)
            try:
                uuid.UUID(returned_id)
            except ValueError:
                pytest.fail(f"Expected UUID for invalid input '{invalid_id}', got: {returned_id}")

    def test_request_id_truncates_long_values(self, public_client):
        """Test that overly long request IDs are truncated."""
        # Create a valid but very long request ID (over 128 chars)
        long_id = "a" * 200

        response = public_client.get(
            "/health", headers={"X-Request-ID": long_id}
        )
        assert response.status_code in [200, 503]

        returned_id = response.headers.get("X-Request-ID")
        # Should be truncated to max 128 chars
        assert len(returned_id) <= 128
        # Should still use the truncated version (first 128 chars)
        assert returned_id == long_id[:128]

    def test_request_id_accepts_valid_formats(self, public_client):
        """Test that various valid request ID formats are accepted."""
        valid_ids = [
            "simple-id",
            "UPPERCASE-ID",
            "mixed_Case.123",
        ...
        for valid_id in valid_ids:
            response = public_client.get(
                "/health", headers={"X-Request-ID": valid_id}
            )
            assert response.status_code in [200, 503]
