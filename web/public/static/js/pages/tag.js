    ...
    return {
        ...
        async init() {
            ...
            try {
                ...
                this._tagName = this.tag.name || '';
                this._tagNameWithHash = '#' + this._tagName;

                if (videosRes.ok) {
                    const data = await videosRes.json();
                    this.videos = (data.videos || []).map(v => this.enrichVideo(v));
                    const count = this.videos.length;
                    this.announcement = 'Tag ' + this.tag.name + ' with ' + count + ' video' + (count === 1 ? '' : 's');
                } else {
        ...
        enrichVideo(video) {
            ...
            // Enrich tags with precomputed name
            const enrichedTags = (video.tags || []).map(t => ({
                ...t,
                _name: '#' + t.name
            }));
            return {
                ...
                _hasTags: enrichedTags.length > 0,
                _tags: enrichedTags
            };
        },
        ...
        updateVideoCountText() {
            const count = this.searchQuery ? this.resultCount : (this.tag?.video_count || 0);
        ...
        updateContentUIState() {
            ...
            this._showContent = !this.loading && !!this.tag;
        ...
        updateEmptyStateText() {
            if (this.searchQuery) {
                ...
                this._emptyStateTitle = 'No videos with this tag yet';
        ...
        getEmptyStateTitle() {
            return this.searchQuery ? 'No videos match your search' : 'No videos with this tag yet';
        },
